    ...
    <div>
      ...
      {!data ? (
        <div className="fixed-top text-center mt-5">
          <h3>
            <Badge pill="pill" variant="secondary">
              Loading...
            </Badge>
          </h3>
        </div>
        ...
        <CardColumns className="mt-2 mr-2 ml-2">
          {data.map(coin => (
            <Card
              ...
              key={coin.id}
              onClick={() =>
                switchBase(coin.id, coin.symbol, coin.current_price)
              }
            >
              ...
              {baseCurrency != "USD" && (
                <div>
                  <Comparison
                    ...
                    priceBase={basePrice}
                    priceCurrent={coin.current_price}
                  />
      ...
      {baseCurrency != "USD" && (
        <div className="fixed-bottom text-right mb-2">
          <h3>
            <Badge
              pill="pill"
              variant="primary"
              onClick={() => switchBase("USD", "USD", 1)}
            >
              <MdRefresh /> {baseSymbol.toUpperCase()} Selected
            </Badge>
          </h3>
        </div>
      )}
