...
export default function Comparison(props) {
  ...
  const fetchHistory = async (coinId, dateRange) => {
    const historicPrice = await coinGeckoClient.coins.fetchHistory(coinId, {
      date: dateRange
    });
  ...
  async function gecko(i) {
    ...
    if (b != 0 && baseSymbol == currentSymbol) {
      ...
      setDataBase(result => [usdPercentChange.toFixed(2), ...result]);
    } else {
      setDataBase(result => [0, ...result]);
    }
    ...
    if (t == 0) {
      ...
      setData(result => [percentChange.toFixed(2), ...result]);
    }
  }

  useEffect(() => {
    async function runLoop() {
      let ranges = [1, 7, 30, 90, 180, 270, 360];
        ...
        <div>
          <h5>
            <Badge pill="pill" variant="primary">
              {baseSymbol.toUpperCase()} Selected
            </Badge>
          </h5>
